...
#define MAXNODES 10 //number of topics should only have 6 topics
...
void MQTTClientCallback(int32_t event, void *metaData, uint32_t metaDateLen, void *data, uint32_t dataLen)
{
    ...
    switch((MQTTClient_EventCB)event)
    {
        ...
        case MQTTClient_RECV_CB_EVENT:
        {
            ...
            strncpy(&queueElement.payload, data, PAYLOAD_STRING_LENGTH);
            //snprintf(&queueElement.payload, PAYLOAD_STRING_LENGTH, "%s", data );
            int i;
            for(i = 0; i < receivedMetaData->topLen; i++) {
                queueElement.topic[i] = receivedMetaData->topic[i];
            }
...
// Uses linked list to handle topics
// why?
void *MQTTAppThread(void *threadParams)
{
    ...
    while(1)
    {
        ...
        {
            dbgEvent( MQTT_APP_THREAD_EVENT_DESTROY );
...
int MQTT_IF_Init(MQTT_IF_InitParams_t initParams)
{
    ...
    mMQTTContext.moduleMutex = pthreadMutex;
...
Node* MyNodeAlloc() {
    ...
    if(nodesAllocated == MAXNODES) {
        ...
        nodesAllocated++;
        LOG_TRACE("Number of Nodes Allocated %d\r\n", nodesAllocated);
        return(&(Nodes[nodesAllocated-1]));
    }
}
char* MyTopicAlloc() {
    static char Topics[MAXNODES][MAX_STRING_SIZE];
    static int topicsAllocated = 0;
    if(topicsAllocated == MAXNODES) {
        LOG_ERROR("Too many Nodes and Topics \r\n");
        pthread_mutex_unlock(&(mMQTTContext.moduleMutex));
        return NULL;
    }
    else {
        topicsAllocated++;
        LOG_TRACE("Number of Topics Allocated %d\r\n", topicsAllocated);
        //return(Topics+topicsAllocated-1);
        return(&(Topics[topicsAllocated-1]));
    }
}
int MQTT_IF_Subscribe(MQTTClient_Handle mqttClientHandle, char* topic, unsigned int qos, MQTT_IF_TopicCallback_f topicCB)
{
    dbgEvent( MQTT_IF_SUB_ENTER );
    int ret = 0;
    ...
    */
    ...
    topicEntry->topicParams.topic = MyTopicAlloc();
    ...
    if(mMQTTContext.moduleState == MQTT_STATE_CONNECTED){
        ...
        if(ret < 0){
            ...
            //stopicEntry->topicParams.topic);
            //free(topicEntry);
        }
    }

    pthread_mutex_unlock(&(mMQTTContext.moduleMutex));
    dbgEvent( MQTT_IF_SUB_EXIT );
    return ret;
}
